...
// CRONDATA
//   [#] = [0]=Empty, [1]=Switch1, [2]=Switch2, [3]=Switch3, [4]=Switch4
//   [#][0] = Incremental number for looping
//   [#][1] = Switch Number
//   [#][2] = CronDurationMins
//   [#][3] = CronSec - seconds between (0-59) & -1=*
//   [#][4] = CronMin - minutes between (0-5`9) & -1=*
//   [#][5] = CronHour - hours between (0-23) & -1=*
//   [#][6] = CronDay - day of the month (1-31) & -1=*
//   [#][7] = CronMonth - month of the year (1-12) & -1=*
//   [#][8] = CronWeekDay - day of the week 9SMTWTFS(0-6 with 0=Sunday) & 0=Ignore

int CRONCOUNT = 20;
int CRONDATA [20][9] { //initialize to zero
  {0, 0, 0, 0, 0, 0, 0, 0, 90000000},
  {1, 1, 0, 0, 0, 0, 0, 0, 90000000},
  {2, 2, 10, 0, 30, 17, -1, -1, 91111111},
  {3, 3, 1, 30, 32, 12, 24, 2, 90000000 },
  {4, 4, 840, 0, 0, 6, -1, -1, 91111111 },
  {5, 0, 0, 0, 0, 0, 0, 0, 90000000}
};
...
void SwitchCronFunction(){
  int i=0;
  for (i = 1; i < CRONCOUNT; ++i) {
    // CRONDATA
    //   [#] = [0]=Empty, [1]=Switch1, [2]=Switch2, [3]=Switch3, [4]=Switch4
    //   [#][0] = Switch Number
    //   [#][1] = CronDurationMins
    //   [#][2] = CronSec - seconds between (0-59) & -1=*
    //   [#][3] = CronMin - minutes between (0-5`9) & -1=*
    //   [#][4] = CronHour - hours between (0-23) & -1=*
    //   [#][5] = CronDay - day of the month (1-31) & -1=*
    //   [#][6] = CronMonth - month of the year (1-12) & -1=*
    //   [#][7] = CronWeekDay - day of the week 9SMTWTFS(0-6 with 0=Sunday) & 0=Ignore
    // SWITCHDATA
    //   [#][7] = TimerStartTime
    //   [#][8] = TimerTime

    if ( CRONDATA[i][1] > 0 and CRONDATA[i][0] <= SWITCHCOUNT-1 ) {
      ...
      if ( (Time.second(time) == SWITCHDATA[i][11] or SWITCHDATA[i][11] == -1) and
          ...
          Particle.publish("SwitchCronFunction", String::format("Matched - B - sec:%i, min:%i, i:%i, SWITCHDATA[i][7]:%i, SWITCHDATA[i][11]:%i,weekdayMatcg:%i",Time.second(time),Time.minute(time),i,SWITCHDATA[i][10],SWITCHDATA[i][11],weekdayMatch(Time.day(time), SWITCHDATA[i][14])));
          SWITCHDATA[i][7]=time;
          // Turn ON Relay attached to D#.  #=i
          WriteDigitalPin(SWITCHDATA[CRONDATA[i][0]][0], 0);
          SWITCHDATA[CRONDATA[i][0]][2] = 0;
          CloudSwitchTimer(CRONDATA[i][0], CRONDATA[i][1]);
      }
    }
  }
}
